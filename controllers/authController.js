...
exports.postForgotPassword = async (req, res) => { 
    ...
    try {
        const user = await db.oneOrNone('SELECT * FROM users WHERE email = $1', [email]);
        if (!user) {
            return res.render('pages/forgot_password', { message: 'Email not found' });
        }   
        ...
        res.render('pages/forgot_password', { message: 'An error occurred while processing your request. Please try again.' });
    }
}
...
exports.resetPassword = async (req, res) => {
    ...
    try {
        ...
        if (!user) {
            return res.render('pages/reset_password', { message: 'Invalid or expired reset link', token: '' });
        }
        ...
        res.render('pages/reset_password', { message: 'Password reset successful! You can now log in.', token: '' });
    } catch (error) {
        console.log('Error during password reset:', error);
        res.render('pages/reset_password', { message: 'An error occurred while resetting your password. Please try again.', token: '' });
    }
}
